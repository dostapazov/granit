...
//---------------------------------------------------------------------------
__fastcall TOpcConfigEditor::TOpcConfigEditor(TComponent* Owner)
	: TForm(Owner),opc_server(NULL),opc_browser(NULL),opc_group(NULL)
{
...
{
  if(iprops.IsBound()) iprops.Reset();
  if(opc_browser) { delete opc_browser; opc_browser = NULL;}
  if(opc_group  ) { delete opc_group  ; opc_group   = NULL;}

  if(opc_server ) {delete opc_server;opc_server = NULL;}
...
{
  wchar_t * state_text[]   = {L"Неизвестно",L"Работает",L"Ошибка запуска",L"Не сконфигурирован",L"Приостановлен",L"",L""};
      ...
      while(S_OK == opc_browser->item_next(str))
            {
              ...
              folder_node->Data = FOLDER_NODE_ATTR;
              OpcServerItemsTree->Items->AddChild(folder_node,UnicodeString())->Data = FOLDER_TEMPL_ATTR;
            }
    }
   ...
   if(i<(DWORD)opc_item_props_view->Items->Count)
    ...
    UnicodeString prop_str;
     ...
     else
     {
       ...
       switch(*prop_ids)
       {
         case OPC_PROPERTY_DATATYPE: prop_str = variant_type_name((DWORD)ov); break;
         default:  prop_str = ov; break;
       };
     }
...
bool    __fastcall TOpcConfigEditor::identfy_divisor(TTreeNode *item_node)
{
  DWORD    prop_count = 0;
  LPDWORD  prop_ids   = NULL;
  LPWSTR * prop_descr = NULL;
  LPWORD   prop_data_types = NULL;
  UnicodeString opc_id ;
  UnicodeString  tdiv  = L'.';
  UnicodeString  tpref ;
  opc_id = get_item_opc_id(item_node,tdiv,tpref);
  if(!iprops.IsBound()) iprops = *opc_server;

  if(iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types))
    {
      tdiv = L'\\';
      opc_id = get_item_opc_id(item_node,tdiv,tpref);
     if(iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types))
      {
       ...
       if(iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types))
       {
        ...
        if(iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types))
        {
          ...
          if(iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types))
          {
           std::auto_ptr<TSelItemDivPref>  dlg (new TSelItemDivPref(this));
           do{
             if(dlg->ShowModal() != mrOk) return false;
             tpref = dlg->Edit1->Text;
             tdiv  = dlg->Edit2->Text;
             opc_id = get_item_opc_id(item_node,tdiv,tpref);
             }while( iprops->QueryAvailableProperties(opc_id.c_str(),&prop_count,&prop_ids,&prop_descr,&prop_data_types));
          }
        }
       }
      }
...

{
  ...
  if(Node)
  {
   ...
   wchar_t * fa_names[] = {L"Дискретные",L"Аналоговые",L"Интегральные"};
...
{
  bool ret = false;
  if(OpcServerItemsTree->Selected && !OpcServerItemsTree->Selected->Data && OtdTree->Selected)
    {

      ret =  !is_group_root(OtdTree->Selected);
...
bool __fastcall TOpcConfigEditor::add_gkopc_item(UnicodeString id,UnicodeString access_path,group_param_t & gp,gkopc_items_vector::iterator & ptr)
{
...
gkopc_items_vector::iterator __fastcall TOpcConfigEditor::add_gkopc_item(UnicodeString id,UnicodeString access_path,group_param_t & gp,TTreeNode * opc_node,TTreeNode * otd_node)
{
...
{
  if(!is_group(dst_node)) dst_node = dst_node->Parent;
   ...
   if(dst_node->Count)
     ...
     add_gkopc_item(get_item_opc_id(src_node,divisor,prefix),UnicodeString(),gp,src_node,otd_node);
}
...
void __fastcall TOpcConfigEditor::OpcServerItemsTreeCustomDrawItem(TCustomTreeView *Sender,
...
void __fastcall TOpcConfigEditor::OpcServerItemsTreeAddition(TObject *Sender, TTreeNode *Node)
{
  if(Node && Node->Data != (LPVOID)-1)
   {

     gkopc_item_def_vcl oi(get_item_opc_id(Node,divisor,prefix),UnicodeString(),group_param_t());
     gkopc_items_vector::iterator ptr;
     if(opc_items.find(oi,ptr))
        {
         ptr->opc_node = Node;
         Node->Data = (LPVOID)TRUE;
        }
   }
}
//---------------------------------------------------------------------------
...
UnicodeString  __fastcall TOpcConfigEditor::get_item_opc_id(TTreeNode * node,UnicodeString divisor,UnicodeString prefix)
{
   UnicodeString str;
   if(node)
   {
      if(!(divisor.Length() == 1 && L' ' ==  *divisor.c_str()))
      {
      str = get_item_opc_id(node->Parent,divisor,prefix);
      if(str!=prefix && !divisor.IsEmpty())
            str+=divisor;
      }
      str+= node->Text.c_str();
      //str.cat_printf(_T("%s"),str.IsEmpty() ? _T() : divisor ,node->Text.c_str());
   }
   else
   {
     return prefix;
...
void __fastcall TOpcConfigEditor::OpcServerItemsTreeExpanding(TObject *Sender, TTreeNode *Node,
          bool &AllowExpansion)
{
...
void __fastcall TOpcConfigEditor::miCopyTagNameClick(TObject *Sender)
{
  ...
  if(node)
     {
      tutr_tag->Text = get_item_opc_id(node,this->divisor,this->prefix);
      changes = true;
     }
...
void    __fastcall TOpcConfigEditor::do_export(UnicodeString file_name)
{
  ...
  if(strings->Count)  strings->SaveToFile(file_name);
}
...
void __fastcall TOpcConfigEditor::StatusBar1DrawPanel(TStatusBar *StatusBar, TStatusPanel *Panel,
          const TRect &Rect)
{
