                                 ...
                                 lpgr_proto gp)
{
  ...
  if(buff && bsz>=GRSC_MIN_FRAME_SIZE)
  {
   if(gp && gp->dw_size == sizeof(*gp))
   {
    ...
    DWORD pdl = 0; /*Длина данных протокола frame data length*/
    switch(gp->type->ktype)
    {
     case GRKT_CALL    :/*Все вызовы  кроме ВФА имеют 8 байт данных all calls except C(all) F(unctional) A(ddress) has 8 bytes of data */
...
/*Формирование кадра ТУ Возвращает длину кадра*/
