...
void __fastcall main_dblow::report_tutr_sended(otd_addr & addr,DWORD obj , otd_tutr & tutr,modem_addr & src)
{
     ...
     *PUname = *CPname = *op_name = *modem_text = 0;
     ...
     {
      case OTD_TUOP_ON      : safe_strcpy(op_name,L"ВКЛ/+");break;
      case OTD_TUOP_OFF     : safe_strcpy(op_name,L"ОТКЛ/-");break;
      case OTD_TUTROP_CANCEL: safe_strcpy(op_name,L"ОТМЕНА");break;
     }

//     switch(tutr.command&OTD_TUTR_CMDMASK)
//     {
//       case OTD_TUTR_CMDSELECT   : safe_strcpy(op_name,L"ВЫБОР ОБЪЕКТА");break;
//       case OTD_TUTR_CMDENABLE   : safe_strcpy(op_name,L"РАЗРЕШЕНИЕ ТУ/ТР");break;
//       case OTD_TUTR_CMDDESELECT : safe_strcpy(op_name,L"ОТМЕНА ВЫБОРА");break;
//     }
