/*
 ...
 Описание протокола для счётчика ВТД
 
*/

#ifndef _STP_VTDPROTO_INC
#define _STP_VTDPROTO_INC
...
#define VTD_DEVICE_MAX_NUMBER 254 // Максимальный номер Устройства

/*Типы счётчиков*/
#define VTD_TYPE_STD   0
#define VTD_TYPE_VG    1
#define VTD_TYPE_Y     2


typedef struct _vtd_query_kadr
{
 BYTE dev_number;   //Номер устройства
 BYTE command;      // команда в документации обозначена как КИ - код информации
 BYTE Npipe;        //Номер трубы/потребителя
 BYTE Npar;         //Номер параметра
 BYTE zero;         //Резерв
 BYTE CNTpar;       //Количество параметров
 WORD crc;
} vtd_query_kadr,*lpvtd_query_kadr;
...
{
 vtd_digits      c_code;       // Код счетчика
 vtd_date        curr_date;    // текущая дата
 vtd_time        curr_time;    // Текущее время
 vtd_date        prevrep_date; // дата предыдущего отчёта
 vtd_date        lastrep_date; // дата последнего  отчета
 vtd_date_time   start_dt[10]; // времена пусков труб
 WORD            crc;
...
typedef struct _vtd_pipe_curr_param
{
 vtd_time      time;
 float         pressure [10];
 ...
 WORD          crc;
}vtd_pipe_curr_param,*lpvtd_pipe_curr_param;

typedef struct _vtd_user_curr_param
{
 ...
 WORD           crc;
}vtd_user_curr_param,*lpvtd_user_curr_param;
