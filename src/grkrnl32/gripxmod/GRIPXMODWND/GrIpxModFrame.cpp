...
void     __fastcall TGrIpxModuleWnd::monitor_grnet_kadr (LPGRNET_KADR kadr,DWORD len, BOOL tx)
{
   if(check_monitor_filtr(kadr,len) && kadr->command!= CmCheckConnect && !kadr->receipt)
   {
    ...
    GetLocalTime(&st);
    AnsiString str;
    str.printf("%02d:%02d:%02d:%03d %s",st.wHour,st.wMinute,st.wSecond,st.wMilliseconds,    tx ? "Передача : ":"Прием    : ");
    monitor_string(str.c_str(),0,color);
    str.printf("кадр №%05d ID-%03X net_cmd=%05d попытка %02d ",kadr->kadr_num,kadr->id,kadr->command,(DWORD)kadr->retry_num);
    monitor_string(str.c_str(),0,color);
    lpobject_tu otu = (lpobject_tu) (kadr->data);
    switch(kadr->command)
    {
     case CmConnect      : monitor_string("Подключение...",NULL,RGB(78,120,146));
          break;
     case  CmDisconnect  : monitor_string("Отключение",NULL,RGB(255,0,0));
          break;
     case  CmBeginTU     :
                           str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","Начало",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmResetTU     :
                           str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","Отмена выбора",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmQueryTU     : str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","Выбор",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmTuFalse     :
                           str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","Неуспешное",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,RGB(255,0,0));
          break;
     case  CmTuOn        :
                           str.printf("%s  %03d.%02d.%02d - %02d ","Команда ВКЛЮЧИТЬ/БОЛЬШЕ",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmTuOff       : 
                           str.printf("%s  %03d.%02d.%02d - %02d ","Команда ОТКЛЮЧИТЬ/МЕНЬШЕ",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmTrNorm      : monitor_string("");
          break;
     case  CmTuCancel    :
                           str.printf("%s  %03d.%02d.%02d - %02d ","Команда ОТМЕНА",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmEnableTU    :
                           str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","РАЗРЕШЕНИЕ",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmDisableTU   :
                           str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","ЗАПРЕЩЕНИЕ",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
          break;
     case  CmErrorTU     : monitor_string("ОШИБКА ТУ",NULL,RGB(255,0,0));
          break;
     case  CmActivateTR  : str.printf("%s ТУ/ТР %03d.%02d.%02d - %02d ","Активизация",otu->kp,otu->tlm_type,otu->sb,otu->obj);
                           monitor_string(str.c_str(),NULL,color);
     ...
     case  CmQueryCp     :
     case  CmQueryFa     :
     case  CmQueryGroup  : monitor_string("Вызов данных");
          break;
...
void     __fastcall TGrIpxModuleWnd::monitor_gratony_kadr( lp_gratony_kadr  kadr,DWORD color)
{
 AnsiString str;
 char       mon_text[1024];
 str.printf("КП-%03d ГРУППА-%03d ",(DWORD)kadr->numCp,(DWORD)kadr->numGrp);
