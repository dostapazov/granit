...
__fastcall THistoryForm::THistoryForm(TComponent* Owner,TGKTlmViewer* vm,UnicodeString _kadr_text,UnicodeString _period_text)
	: TForm(Owner)
        ,viewer_module(vm)
{
 ...
 HistView->Color = (TColor)viewer_module->get_params().ready_colors.bkgnd_stable;
}
//---------------------------------------------------------------------------
  ...
  if(is_null)
    ...
    else
    {
      if(!tmval)
       ...
       {
        text = TTlmViewerForm::get_time_text(tmval,time_sep,msec);
       }
    }
}
    ...
    if(on_get_record)
    {
     if(SameValue(val,double(.0)) || SameValue(val,double(1.0)))
     {
       if(record.rec_id != rec_id )
           on_get_record(this,rec_id,false,record);
...
void __fastcall THistoryForm::get_state_view_text ( DWORD state,UnicodeString & text,bool is_null)
{
   if(is_null)
    ...
    {
     text = TTlmViewerForm::get_state_view_text(state);
    }
}
...
void __fastcall THistoryForm::get_state_tu_text ( DWORD state,UnicodeString & text,bool is_null)
{
   if(is_null)
    ...
    {
     text = TTlmViewerForm::get_state_tu_text(state);
    }
}
...
void __fastcall THistoryForm::tbRequestAlarmPeriodClick(TObject *Sender)
{
  if(request_proc && HistView->Selected && storage_created())
  {
     ...
     LONGLONG  beg_tm = rec_data.ch_time - MSEC_NS100(viewer_module->get_params().history_time_before  );
     LONGLONG  end_tm = rec_data.ch_time + MSEC_NS100(viewer_module->get_params().history_time_after   );
     ...
     while(rec_no<(DWORD)HistView->Items->Count)
     {
      ...
      if(rec_data.ch_time<end_tm)
      {
        if(rec_data.rec_id == rec_id && !SameValue(value,(double)rec_data.rvalue) )
          {
           end_tm = rec_data.ch_time + MSEC_NS100(viewer_module->get_params().history_time_after);
...
string make_std_string(UnicodeString src)
{
 ...
 DWORD __fastcall THistoryForm::comtrade_prepare(Tcomtrade& cmt,int resample)
 {
    ...
    while((rec_no<storage_rec_count) && (rec_data.ch_time == 0/*(QWORD)start_time*/))
    {
      ...
      mdb_record_ex rec;
      if(on_get_record(this,rec_data.rec_id,false,rec))
      {
        UnicodeString rec_name;
        on_get_text(this,rec.rec_id,rec_name);
...
void __fastcall THistoryForm::actExportComtradeExecute(TObject *Sender)
{
  ...
  cp->ProgramName->Text = viewer_module->get_params().export_viewer.c_str();
  if(cp->ShowModal()==mrOk)
  {
    ...
    string cmt_fname = make_std_string(UnicodeString(viewer_module->get_params().export_folder.c_str())+cp->ctFile->Text);
    ...
    if(cp->CheckBox1->Checked && !cp->ProgramName->Text.IsEmpty())
      {
        file_name = UnicodeString(L"\""+UnicodeString(viewer_module->get_params().export_folder.c_str())+cp->ctFile->Text+UnicodeString(L".cfg")+L"\"");
...
void __fastcall THistoryForm::HistViewData(TObject *Sender, TListItem *Item)
{
  ...
  if(temp_data_storage && Item->Index<(int)temp_data_storage->get_record_count())
  {
   ...
   temp_data_storage->get_record(Item->Index,&rec_data,sizeof(rec_data));
   UnicodeString text;
   on_get_text(this,rec_data.rec_id,text);
   Item->Caption = text;
          ...
          bool Selected)
{
  ...
  if(Selected && Item && temp_data_storage && Item->Index<(int)temp_data_storage->get_record_count())
  {
   ...
   if(rec_data.state)
      str+=TTlmViewerForm::get_state_text(rec_data.state);
  }
...
void __fastcall THistoryForm::actAddSeriesExecute(TObject *Sender)
{
   ...
   if(item)
   {
    ...
    if(viewer_module->__get_record(rec_id,false,rec) && !series.count(rec_id))
    {
      is_discrete =     (rec.is_discrete());
      TFastLineSeries  * fl = new TFastLineSeries(Chart1);
...
void __fastcall THistoryForm::actRemoveSeriesExecute(TObject *Sender)
{
   ...
   if(item)
   {
    ...
    temp_data_storage->get_record(item->Index,&rec_data,sizeof(rec_data));
    DWORD rec_id = rec_data.rec_id;
    if(series.count(rec_id))
    {
      TChartSeries * fl = series[rec_id];
      Chart1->RemoveSeries(fl);
      delete fl;
      series.erase(rec_id);
    }
   }
 ...
 void __fastcall hist_stat_t::set_value(DWORD _rec_id,double value,QWORD tm)
 {
   ...
   if(rec_id == _rec_id)
   {
       ...
       if(count)
       {
        ...
        rec_avg     = rec_integral/time_lenght;
 ...
 void __fastcall THistoryForm::make_staticstic()
 {
   if(temp_data_storage && temp_data_storage->is_good())
   {
    ...
    while(ptr!=end)
    {
         ...
         UnicodeString text;
         on_get_text(this,hs.rec_id,text);
         item->Caption = text;
...
TColor __fastcall THistoryForm::get_color_for_list_view(DWORD diag,DWORD state)
{
   TColor color;
   if(diag)
      color =  (TColor)viewer_module->get_ready_colors().name_bad ;
      else
      {
       if(state & MDBR_STATE_ALARM)
       color =  (TColor)viewer_module->get_ready_colors().state_alarm;
       else
       {
       if(state & MDBR_STATE_DANGER)
         color =  (TColor)viewer_module->get_ready_colors().state_danger;
          else
         color = (TColor)viewer_module->get_ready_colors().name_good ;
       }
      }
  return color;
}
...
void __fastcall THistoryForm::export_HTML_history(KeRTL::TFileStream & fs)
{
     ...
     while(i< rec_count )
     {
      ...
      if(rec_data.ch_time)
      {
         on_get_text(this,rec_data.rec_id,text[0]);
