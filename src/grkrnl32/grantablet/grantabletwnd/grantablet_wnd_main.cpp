...
gkhandle_wnd_map<TGranTabledFrm>  wnd_map;

LRESULT WINAPI module_main(DWORD cmd,LPARAM p1,LPARAM p2)
{
  LRESULT ret = 0;
  LPMODULE_WINDOW mw;
  TGranTabledFrm* mf = dynamic_cast<TGranTabledFrm*>(TGKModuleForm::get_form((HWND)p1));
  switch(cmd)
  {
   ...
   case MWCM_CREATE_WINDOW:
    ...
    if(mf == NULL)
    {
     mf = new TGranTabledFrm(Application,mw->parent_window,mw->mod_handle);
     wnd_map.add_form(mf);
    }
