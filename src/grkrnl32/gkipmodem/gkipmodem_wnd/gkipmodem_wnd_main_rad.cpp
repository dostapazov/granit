...
gkhandle_wnd_map<TGKModuleForm>  wnd_map;
...
LRESULT WINAPI module_main(DWORD cmd,LPARAM p1,LPARAM p2)
{
  ...
  TGKModuleForm* mf = TGKModuleForm::get_form((HWND)p1);// dynamic_cast<TGKModuleForm*>();
  switch(cmd)
  {
   ...
   case MWCM_CREATE_WINDOW:
    ...
    {
     try{
         mf = new TGkIPModemFrm((TComponent*)NULL,mw->parent_window,mw->mod_handle);
                  //TGKIpModemForm
         wnd_map.add_form(mf);
        }
        catch(Exception & x)
