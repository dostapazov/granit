        ...
        : TGKModuleForm(Owner,owner_hwnd,handle)
{
 ...
 Label30->Caption = L"ООО \"Компания промышленной автоматизации\"";
 #endif
...
TTreeNode * __fastcall TModemmerFrm::add_line_node(TTreeNode * modem,LPMODEM_LINE_PROPS mlp)
{
  ...
  {
   AnsiString s ;
   AnsiString line_info;
   if(mlp->line_text_len)
      s = mlp->line_text;
      else
      {
      s = load_rcstring(ModuleInstance,IDS_LINE);
      s.cat_printf(" № %d",line_info.c_str(),(DWORD)mlp->addr.line);
      }
...
void        __fastcall TModemmerFrm::on_modem_change()
{
 ...
 MiSettings->Enabled = enable_controls;
 PropBtn-> Enabled   = enable_controls;
 MiView->Enabled     = enable_controls;
 ViewBtn-> Enabled   = enable_controls;
 RefreshBtn->Enabled = enable_controls;
 MiRefresh->Enabled  = enable_controls;
 MnPanel->Visible    = enable_controls;
 SetMNBtn->Visible   = enable_controls;
 ToolBar1->Update();
...
void        __fastcall TModemmerFrm::update_line_state(MODEM_ADDR & ma ,DWORD line_state)
{
  ...
  {
   str = load_rcstring(ModuleInstance,IDS_LINE_STATE);
...
void __fastcall TModemmerFrm::on_modemmer_txrx(LPMPROTO_HEADER mph,DWORD sz,DWORD code)
{
        ...
        if((rx_mon_enabled && code == MNCODE_MODEMMER_RX)||(tx_mon_enabled && code == MNCODE_MODEMMER_TX) || (redir_mon_enabled && code == MNCODE_MODEMMER_REDIRECT))
        {
        if(mph)
        {
         DWORD color;         
...
void __fastcall TModemmerFrm::monitor_string(AnsiString & s,DWORD color)
{
...
void __fastcall TModemmerFrm::Edit1Change(TObject *Sender)
{
 ...
 {
 short number = _wtoi(Edit1->Text.c_str());
...
void __fastcall TModemmerFrm::MonitorParamClick(TObject *Sender)
{
  ...
  if(filter_enabled)
  {
   filter_modem =  _wtoi(FilterModem->Text.c_str());
   filter_line  =  _wtoi(FilterLine ->Text.c_str());
   filter_fa    =  _wtoi(FilterFA   ->Text.c_str());
  }
}
//---------------------------------------------------------------------------
 ...
 if(str2addr(RedirFrom->Text.c_str(),(LPBYTE)&re->from_addr,sizeof(re->from_addr),'.') &&
   ...
   )
   {
    if(re->from_addr.modem && re->from_addr.modem<MODEM_ADDR_MAXVALUE && re->to_addr.modem && re->to_addr.modem<MODEM_ADDR_MAXVALUE)
      {
       re->fa = RedirFA ->Text.Length() ? (BYTE)_wtoi(RedirFA->Text.c_str()): FA_ALL;
       DWORD opt = 0;
...
{
  int value = _wtoi(MonLinesCount->Text.c_str());
